...
async function fetchPredictions(routeId) {
  ...
  // if (routeId !== "ALL") {
  //   h1Element.textContent = `${busNameMap[routeId]} ETA at CCTC`;
  // } else {
  //   h1Element.textContent = `ETA at CCTC for all routes`;
  // }
  ...
  try {
    const response = await fetch(`http://127.0.0.1:5000/predictions?route_id=${routeId}`);
                ...
                .map((prediction) => {
                  ...
                  return `
                    <tr>
                      ...
                      <td>${routeId === "ALL" ? prediction.route_id : prediction.vehicle_id}</td>
...
function startAutoRefresh(routeId) {
  ...
  autoRefreshInterval = setInterval(() => fetchPredictions(routeId), refreshInterval);
}
